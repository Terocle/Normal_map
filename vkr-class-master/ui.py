    ...
    def load_image(self):
        ...
        if path:
            try:
                ...
                QMessageBox.warning(self.window, "Неверный файл",
                                    "Выбранный файл не является допустимым изображением.")
                return

            width, height = image.size
            if width < 64 or height < 64 or width > 2048 or height > 2048:
                QMessageBox.warning(self.window, "Invalid Image Size",
                                    "Размер изображения должен быть между 64x64 и 2048x2048 пикселей.")
                return
